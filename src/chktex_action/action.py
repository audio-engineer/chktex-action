...
from chktex_action.pull_request import get_pull_request_number, process_pull_request
...
def run() -> int:
    ...
    token = Auth.Token(env.github_token)
    github = Github(auth=token)
    repository = github.get_repo(env.github_repository)
    latest_commit = repository.get_branch(env.head_ref).commit

    step_summary_path = env.step_summary
    is_lint_all_enabled = env.lint_all
    github_workspace_path = env.github_workspace

    pull_request = None

    if is_pull_request and not is_lint_all_enabled:
        pull_request = repository.get_pull(
            get_pull_request_number(env.github_event_path)
        )
    ...
    if not files:
        ...
        write_step_summary(step_summary_path, "No .tex files found.")
    ...
    write_step_summary(step_summary_path, step_summary)
